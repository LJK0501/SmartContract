...
contract MyAuction {
    ...
    modifier an_ongoing_auction() {
        require(block.timestamp <= auction_end, "경매가 이미 종료되었습니다");
        _;
    }

    // 경매 소유자만 호출할 수 있는 modifier
    modifier only_owner() {
        require(msg.sender == auction_owner, "경매 소유자만 호출할 수 있습니다");
        _;
    }
    ...
    function bid() public payable an_ongoing_auction returns (bool) {
        require(msg.value > 0, "입찰 금액은 0보다 커야 합니다");
        require(bids[msg.sender] + msg.value > highestBid, "더 높은 금액으로 입찰해야 합니다");
        require(msg.sender != auction_owner, "경매 소유자는 입찰할 수 없습니다");
    ...
    function deactivateAuction() external only_owner {
        require(block.timestamp > auction_end, "경매가 아직 진행 중입니다");
        STATE = auction_state.CANCELLED;
    ...
    function withdraw() public returns (bool) {
        ...
        require(amount > 0, "출금할 금액이 없습니다");

        // 🔥 핵심 로직: 
        // 경매가 진행 중이면 → 누구나 회수 가능
        // 경매가 끝났으면 → 최고 입찰자가 아닌 사람만 회수 가능
        if (block.timestamp > auction_end) {
            require(msg.sender != highestBidder, "낙찰자는 경매 종료 후 출금할 수 없습니다");
        }

        bids[msg.sender] = 0;
        (bool success, ) = payable(msg.sender).call{value: amount}(""); 
        require(success, "전송에 실패했습니다");
    ...
    function withdrawWinningBid() external only_owner {
        require(block.timestamp > auction_end, "경매가 아직 진행 중입니다");
        require(highestBidder != address(0), "입찰이 없습니다");
        
        uint amount = bids[highestBidder];
        require(amount > 0, "출금할 낙찰금이 없습니다");
    ...
    function withdrawRemainingFunds() external only_owner {
        ...
        require(balance > 0, "컨트랙트에 남은 자금이 없습니다");
